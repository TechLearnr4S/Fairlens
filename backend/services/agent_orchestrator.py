...
async def run_fairness_copilot(audit_id: str, local_disparities=None, local_proxies=None) -> Dict[str, Any]:
    """
    ...
    """
    # 1. Check Cache First (Bypass if no credentials)
    try:
        ...
        logger.warning(f"Could not load cache (likely missing Firebase credentials): {e}")

    try:
        # 2. Fetch required data
        fairness_data = local_disparities if local_disparities else get_fairness_results(audit_id)
        proxy_results = local_proxies if local_proxies is not None else get_proxy_flags(audit_id)
        ...
        # 7. Persist and Cache (Bypass if no credentials)
        try:
            save_copilot_cache(audit_id, final_report)
            save_copilot_summary(audit_id, final_report)
        except Exception as e:
            logger.warning(f"Could not save cache (likely missing Firebase credentials): {e}")
